...
def POE(logdir, target, logging, debug):
    ...
    if (logging == True):
        newlogentry = 'Running strings against: <strong>' + target.filename + '</strong>'
        LOG.WriteLog(logdir, target.filename, newlogentry)

    subproc = subprocess.Popen('strings -a ' + target.filename, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
