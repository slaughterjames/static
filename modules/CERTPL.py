...
'''
***BEGIN DESCRIPTION***
Type: Triage - Description: Retrieves any available data on a target against the CERT.PL malware database.
***END DESCRIPTION***
'''
def POE(POE):
    ...
    if (apikey == ''):
        print (colored('\r\n[x] Unable to execute CERTPL - apikey value not input.  Please add one to /opt/static/static.conf', 'red', attrs=['bold']))
        if (logging == True):
            newlogentry = 'Unable to execute CERTPL - apikey value not input.  Please add one to /opt/static/static.conf'
    ...
    tag0 = ''
    tag1 = ''
    output = POE.logdir + 'CERTPL.json'

    FI = fileio()
    
    print (colored('\r\n[*] Running CERT.PL against: ' + POE.target, 'white', attrs=['bold']))
    ...
    try:        
        ...
        print (colored('[*] CERTPL data had been written to file here: ', 'green') + colored(output, 'blue', attrs=['bold']))
        if ((POE.logging == True) and (POE.nolinksummary == False)):
            newlogentry = 'CERTPL data has been generated to file here: <a href=\"' + output + '\"> CERTPL Output </a>'           
            LOG.WriteSubLog(POE.logdir, POE.targetfilename, newlogentry)
    except:
        print (colored('[x] Unable to write CERTPL data to file', 'red', attrs=['bold']))
        if (POE.logging == True):
            newlogentry = 'Unable to write CERTPL data to file'
            LOG.WriteStrongSubLog(POE.logdir, POE.targetfilename, newlogentry)
        return -1

    try:
        #Open the file we just downloaded
        print ('[-] Reading CERTPL file: ' + output.strip())
