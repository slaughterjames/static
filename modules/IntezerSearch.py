...
'''
***BEGIN DESCRIPTION***
Type: Search - Description: Retrieves any available data for a target against the Intezer database.
***END DESCRIPTION***
'''
def POE(POE):
    ...
    if (POE.logging == True):
        newlogentry = 'Module: IntezerSearch'           
        LOG.WriteStrongLog(POE.logdir, POE.targetfilename, newlogentry)

    if (POE.SHA256 == ''):
        print (colored('\r\n[x] Unable to execute IntezerSearch - hash value must be SHA256.', 'red', attrs=['bold']))
        newlogentry = 'Unable to execute IntezerSearch  - hash value must be SHA256'
    ...
    if (apikey == ''):
        print (colored('\r\n[x] Unable to execute IntezerSearch - apikey value not input.  Please add one to /opt/static/static.conf', 'red', attrs=['bold']))
        if (logging == True):
            newlogentry = 'Unable to execute IntezerSearch - apikey value not input.  Please add one to /opt/static/static.conf'
            LOG.WriteStrongSubLog(POE.logdir, POE.targetfilename, newlogentry)
            POE.csv_line += 'N/A,'
        return -1

    global json
    output = POE.logdir + 'IntezerSearch.json'
    analysis_time = ''
    analysis_url = ''
    family_name = ''
    verdict = ''
    ...
    print (colored('\r\n[*] Running IntezerSearch against: ' + POE.target, 'white', attrs=['bold']))
    ...
    if (response.status_code == 200):
        ...
        try:
            ...
            print (colored('[*] Intezer search report data had been written to file here: ', 'green') + colored(output, 'blue', attrs=['bold']))
            if (POE.logging == True):
                if (POE.nolinksummary == False):
                    newlogentry = 'Intezer search report data has been generated to file here: <a href=\"' + output + '\"> Intezer Summary </a>'           
                    LOG.WriteSubLog(POE.logdir, POE.targetfilename, newlogentry)
        except:
            print (colored('[x] Unable to write Intezer search report data to file', 'red', attrs=['bold']))
            if (POE.logging == True):
                newlogentry = 'Unable to write Intezer search report data to file'
                LOG.WriteSubLog(POE.logdir, POE.targetfilename, newlogentry)
                return -1
